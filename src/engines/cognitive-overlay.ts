...
export function buildThetaCognitiveOverlay(
  ...
  const holdingPattern = behaviorPatterns.find(p => p.titulo.toLowerCase().includes("vencimento") || p.descricao.toLowerCase().includes("segurar"));
  if (holdingPattern) {
    ...
    behaviorInsight = "Historicamente, você costuma manter posições semelhantes até os últimos dias da curva gama, o que nas últimas 4 vezes reduziu seu retorno médio de saída.";
  }

  // 4. Cruzamento com Regras de Ouro (Rule Engine Inject)
  let ruleAlert: string | null = null;
  const triggeredRevisions = userRulesTriggered.find(r => r.regra.includes(daysToMaturity.toString()) || r.regra.toLowerCase().includes("fechar"));
  if (triggeredRevisions) {
    ruleAlert = `Sua Regra Pessoal: "${triggeredRevisions.regra}"`;
  } else if (daysToMaturity < 7) {
    ruleAlert = "Lembrete: Suas regras de sistema recomendam iniciar a reavaliação defensiva antes de DTE < 7.";
  }
