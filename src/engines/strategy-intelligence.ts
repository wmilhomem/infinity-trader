...
export function orchestrateStrategy(
  ...
  if (inter.objetivo === "lateralizacao") {
    if (volLanguage.acaoStatus === "comprar_vol") {
      parts.push(
        `🚨 Paradoxo: Você está vendendo risco (aposta em paralisia), mas a volatilidade histórica subjacente está baixíssima no momento. Risco imenso de expansão de Vega esmagar a estrutura.`,
      );
    } else {
      parts.push(
        `✓ Coerência tática: Apostando em lateralização num pico de volatilidade, colhendo prêmios caros na originação.`,
      );
    }
  }
  ...
  if (expMove > 0) {
    parts.push(
      `O mercado projeta com ~68% de certeza um movimento na ação (para cima ou baixo) de **R$ ${expMove.toFixed(2)}** até o vencimento.`,
    );
  }

  if (pop > 0 && pop < 100 && inter.breakevens.length > 0) {
    parts.push(
      `De acordo com a fronteira dos breakevens lidos, a **Probabilidade de Lucro (PoP)** matemática na data H é de aproximadamente **${pop.toFixed(0)}%**.`,
    );
  }

  if (portfolioTheta > 0) {
    parts.push(
      `⏳ O tempo corre a seu favor. O carregamento estéril diário (Theta) está gerando caixas de R$ ${portfolioTheta.toFixed(2)}.`,
    );
  } else if (portfolioTheta < 0) {
    parts.push(
      `⏳ Cuidado com a anestesia temporal: O decaimento da sua estrutura comprada queima R$ ${Math.abs(portfolioTheta).toFixed(2)} de valor por dia parado.`,
    );
  }
