...
export function interpretVolatility(ctx: VolatilityContext): VolatilityLanguage {
  ...
  let superf = "";
  if (term === "backwardation") superf += "O mercado está com pânico imediato (opções curtas valendo mais que as longas). ";
  else superf += "O prêmio no tempo estrutural está normal para longo prazo. ";
  
  if (skew === "put_heavy") superf += "Existe um forte medo de queda: as Puts de proteção estão desproporcionalmente caras.";
  if (skew === "call_heavy") superf += "Existe euforia: os investidores estão pagando agressivo pelas Calls (Alta).";
  if (skew === "balanced") superf += "O preço do medo vs cobiça no mercado está perfeitamente equilibrado.";

  // Crush Alert (Esmagamento de Prêmio)
  let evtMsg = null;
  if (evento.crushProbability === "alta") {
    evtMsg = "CUIDADO: Há um balanço/evento corporativo iminente. Se você comprar opções secas agora, será esmagado pela queda natural da Volatilidade (Vol Crush) que ocorre no mercado no dia seguinte do anúncio, mesmo acertando a direção da ação.";
  } else if (evento.crushProbability === "media") {
    evtMsg = "Atenção: Evento se aproximando. As opções começam a inchar o prêmio (volatilidade subindo) antecipando um provável balanço corporativo.";
  }
