...
import type { LessonMissao, MissaoOpcao } from "@/lib/lessons";
    ...
    <div className="rounded-2xl border-2 border-primary/50 bg-primary/10 p-6 md:p-8">
      ...
      {!revelado ? (
        ...
        <div className="mt-5 space-y-3">
          <div className={`rounded-xl border p-5 ${escolhida ? TOM[escolhida.tom].box : ""}`}>
            <div className={`text-sm font-semibold ${escolhida ? TOM[escolhida.tom].header : ""}`}>
              {escolhida ? TOM[escolhida.tom].titulo : ""}
            </div>
            ...
            {!acertou && (
              <div className="mt-4 rounded-lg border border-border bg-card/60 p-3 text-sm">
                ...
                <strong>
                  {letra(correta)}) {missao.opcoes[correta].texto}
                </strong>
              </div>
            )}
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            {!acertou && (
              <button
                ...
                className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-border px-5 py-3 text-sm font-medium hover:bg-accent"
              >
            ...
            <button
              onClick={onContinuar}
              className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90"
            >
